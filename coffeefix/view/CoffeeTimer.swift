...
@objc public protocol CoffeeTimerDelegate: class {
...
public class CoffeeTimer: UIView {
    ...
    public func start(beginingValue: Int, interval: TimeInterval = 1) {
        ...
        timer?.invalidate()
        timer = Timer(timeInterval: fireInterval, target: self, selector: #selector(CoffeeTimer.timerFired(_:)), userInfo: nil, repeats: true)
